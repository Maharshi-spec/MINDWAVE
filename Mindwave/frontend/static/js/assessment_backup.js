...
class AssessmentEngine {
    ...
    calculateMetrics(landmarks) {
        ...
        const isTense = (lipHeight / mouthWidth) < 0.15; // Thin lips = Anxiety
        ...
        if (landmarks[1]) {
            ...
            // Calculate Variance (How much is it shaking?)
            if (this.jitterHistory.length > 5) {
                ...
                jitterScore = Math.min(100, (variance / 10) * 50); // High sensitivity to micro-moves
            }
        }
        // 4. STRESS CALCULATION (Strict Matrix)
        // Base Score from Blink Rate (BPM)
        let stress = 0;
        const bpm = eyes.blinkRate;
        
        if (bpm < 10) stress = 10;        // Low/Baseline
        else if (bpm < 20) stress = 20;   // Calm
        else if (bpm < 40) stress = 50;   // Moderate (Alert)
        else if (bpm < 70) stress = 80;   // High (Anxiety)
        else stress = 100;                // Severe (Panic)
        // Modifiers (Boosters)
        if (jitterScore > 30) stress += 15;     // +15 for Shaking
        if (eyes.isBlocking) stress += 30;      // +30 for Shutting Down
        if (isTense) stress += 20;              // +20 for Lip Biting
        // 5. EMOTION CALCULATIONS (With Boosters)
        const lipDist = this.getDistance(landmarks[61], landmarks[291], width, height);
        let happiness = Math.min(100, Math.max(0, (lipDist - 60) * 4)); // Raised threshold to 60px
        const browDist = this.getDistance(landmarks[105], landmarks[334], width, height);
        let anger = Math.min(100, Math.max(0, (50 - browDist) * 4));
        if (isTense) anger += 20; // Tension boosts Anger
        const eyeOpen = this.getDistance(landmarks[159], landmarks[145], width, height);
        let fear = Math.min(100, Math.max(0, (eyeOpen - 12) * 15));
        if (bpm > 40) fear += 30; // Rapid blinking boosts Fear
    ...
    analyzeEyes(landmarks) {
        ...
        if (avgOpen < THRESHOLD) {
            ...
            if (this.isEyeClosed) {
                ...
                if (duration < 500) { // Blink (< 0.5s)
                    this.blinkHistory.push(now);
                }
                this.isEyeClosed = false;
            }
        }
        // Rolling 60s Window for True BPM
        this.blinkHistory = this.blinkHistory.filter(t => now - t < 60000);
        
        return {
            blinkRate: this.blinkHistory.length, // Exact BPM
            isBlocking: this.isEyeClosed && (now - this.blinkStartTime > 1000) // Block > 1s
        };
    }
    
    updateUI(emotions, stressLevel) {
        // Smoothing & Visual Update
